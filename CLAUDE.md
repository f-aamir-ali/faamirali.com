...
- **`factFixes`** are corrections applied to *both*: places where the markdown
  ...
  said "Lead Organizer" against the site's Co-Founder & CEO; the club's Results
  said 12 members and its Solution said ~15 meetings, against the locked 20 and
  ~13.)
...
- **The Surrey Youth AI Summit is "co-founded", and the owner is its CEO.** ⚠️ **This reverses an earlier locked decision** — the site used to say "planned and executed" and explicitly banned "co-founded"; the owner overrode that. Every occurrence has been migrated (`projects.js` intro + `seoDescription`, `site.js` canonical, `llms.txt` ×2, `/about`, `index.astro` meta, and the summit `fileLine`). Yuvraj **Bains** from Panorama Ridge is the co-founder and must stay credited — don't erase him. **Blend the phrasing into the sentence** rather than dropping the words in: third-person surfaces read "he co-founded … and leads it as CEO", first-person reads "I co-founded … and now lead as its CEO". The summit is still stated **confidently** as "the first youth AI summit in Surrey" — no hedge. Never inflate to "first ever AI summit" unqualified; it is the first *youth* summit *in Surrey*.
