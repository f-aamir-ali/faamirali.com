...
The personal site & long-term hub of **F. Aamir Ali** — builder & founder, Grade 12 (Grade 11 when the principal's June 2026 letter was written — the letter keeps its wording), Surrey BC. A fast, static, multi-page Astro site. Every fact on the site must trace back to the four case-study files (the `*_f.md` files under `C:\Users\fateh\Downloads\College Applications\Main Projects\`) or the principal's recommendation letter — nothing goes on the site that isn't sourced from those documents or explicitly given by the owner in conversation.
...
- **`factFixes`** are corrections applied to *both*: places where the markdown
  ...
  Organizer. The club's two old fixes, 12 → 20 members and ~15 → ~13 meetings,
  were removed in Sept 2026 when the source itself was corrected.)
...
- **AI & Innovation Club member count is 20** everywhere (site + PDFs); **~15** bi-weekly meetings. ⚠️ This REVERSED in Sept 2026: the site used to lock ~13 and treat ~15 as the stray figure; the owner confirmed ~15 is correct. `verify.mjs` now forbids "~13 bi-weekly meetings" — don't flip it back.
